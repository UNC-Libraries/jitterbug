...
class TransfersController extends Controller implements HasMiddleware
{
    ...
    public function store(TransferRequest $request)
    {
        ...
        $request->session()->put('alert', ['type' => 'success', 'message' => '<strong>Super!</strong> Transfer was successfully created.']);
    ...
    public function audioImportUpload(Request $request)
    {
        ...
        .'.'.$dataFile->getClientOriginalExtension();
            ...
            $response = ['count' => $import->count(), 'html' => $html];
    ...
    public function audioImportExecute(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (Import::hasErrors($messages)) {
                ...
                $response = ['status' => 'error', 'html' => $html];
            } else {
    ...
    public function update($id, TransferRequest $request)
    {
        ...
        AudioVisualItem::where('call_number', $newInstance->call_number)->first();
            ...
            $this->solrInstances->update([$originalInstance, $newInstance]);
        }
        $this->solrTransfers->update($transfer);

        $request->session()->put('alert', ['type' => 'success', 'message' => '<strong>Success!</strong> Your transfer was updated.']);
    ...
    public function batchUpdate(TransferRequest $request)
    {
        ...
        $originalInstances = [];
        // Update MySQL
        DB::transaction(function () use ($transfers, $pmChanged, $newInstance, $input, &$originalItems,
            &$originalInstances) {
            $transactionId = Uuid::uuid4();
            DB::statement("set @transaction_id = '$transactionId';");
        ...
        $request->session()->put('alert', ['type' => 'success', 'message' => '<strong>Nice!</strong> '.
    ...
    public function destroy($id, Request $request)
    {
        ...
        $request->session()->put('alert', ['type' => 'success', 'message' => '<strong>Gone!</strong> Transfer was successfully deleted.']);
    ...
    public function batchDestroy(Request $request)
    {
        ...
        $request->session()->put('alert', ['type' => 'success', 'message' => '<strong>Voila!</strong> Transfers were successfully deleted.']);
