...
class ItemRequest extends Request {
  ...
  public function rules()
  {
    ...
    if($itemableType === 'AudioItem') {
      $this->addRuleIfNotMixed($rules, 'itemable.size', 'required|max:30');
      $this->addRuleIfNotMixed($rules, 'itemable.trackConfiguration', 'max:100');
      $this->addRuleIfNotMixed($rules, 'itemable.audioBase', 'max:100');
      $this->addRuleIfNotMixed($rules, 'itemable.audioContentDescription','max:1000');
    // Add rules for film items
    } else if ($itemableType === 'FilmItem') {
      $this->addRuleIfNotMixed($rules, 'itemable.filmElement', 'max:255');
      $this->addRuleIfNotMixed($rules, 'itemable.filmBase', 'required|max:255');
      $this->addRuleIfNotMixed($rules, 'itemable.filmColor', 'max:255');
      $this->addRuleIfNotMixed($rules, 'itemable.lengthInFeet', 'required|integer|digits_between:0,5');
      $this->addRuleIfNotMixed($rules, 'itemable.filmStock', 'max:255');
      $this->addRuleIfNotMixed($rules, 'itemable.edgeCode', 'max:255');
      $this->addRuleIfNotMixed($rules, 'itemable.shrinkagePercent', 'numeric|digits_between:0,6');
      $this->addRuleIfNotMixed($rules, 'itemable.canNumber','integer|digits_between:0,6');
      $this->addRuleIfNotMixed($rules, 'itemable.filmContentDescription','max:1000');
    // Add rules for video items
    } else if ($itemableType === 'VideoItem') {
      $this->addRuleIfNotMixed($rules, 'itemable.videoElement', 'max:255');
      $this->addRuleIfNotMixed($rules, 'itemable.videoColor', 'max:255');
      $this->addRuleIfNotMixed($rules, 'itemable.recordingStandard', 'max:255');
      $this->addRuleIfNotMixed($rules, 'itemable.videoContentDescription', 'max:1000');
    }
