    ...
    <table style="margin-top: .5rem;" class="table table-sm table-hover">
      ...
      @foreach ($records as $record)
        <tr>
          ...
          <td><span class="editable" data-id="{{ $record->id }}" data-field="collectionType" role="button">{{ $record->collectionTypeName() }}</span></td>
{{--          <td><span class="editable" data-id="{{ $record->id }}" data-field="legacy" role="button">{{ $record->legacyInReadableformat() }}</span></td>--}}
...
<div id="new-record-form" class="hidden">
  <form class="form-inline">
    ...
    {!! Form::select('collectionTypes', $collectionTypes, null, array('class' => 'form-control form-control-sm')) !!}
    Legacy?
    <input role="button" data-id="{{ $record->id }}" type="checkbox">
...
<div id="edit-collection-type-form" class="hidden">
  <form class="form-inline">
    <input type="text" name="collectionType" class="form-control form-control-sm" maxlength="255" placeholder="Collection Type" autocomplete="off" style="width: 250px;">
    <button class="btn btn-sm btn-secondary popover-submit" type="submit"><i class="fa fa-fw fa-check" aria-hidden="true"></i></button>
    <button class="btn btn-sm btn-secondary cancel-edit"><i class="fa fa-fw fa-ban" aria-hidden="true"></i></button>
  </form>
</div>
