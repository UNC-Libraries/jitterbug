...
{
    protected $requiredVideoImportKeys = [];

    protected $videoImportKeys = [];
    ...
    {
        $this->requiredVideoImportKeys = ['CallNumber', 'FileName', 'IART'];
        $this->videoImportKeys = array_merge($this->requiredVideoImportKeys,
            ['Codec', 'Duration', 'FileSize', 'PreservationChecksum',
    ...
    public function validate()
    {
        ...
        foreach ($this->data as $row) {
          ...
          && ! empty($row[$key]) && in_array($row[$key], $fileNames)) {
                ...
                } elseif ($key === 'FileName' && ! empty($row[$key])) {
                    $fileNames[] = $row[$key];
                }
